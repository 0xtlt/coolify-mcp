...
export function registerDatabaseStorageTools(
	...
	server.tool(
		...
		"List all persistent storage mounts for a Coolify database",
	...
	if (isToolAllowed("coolify_create_database_storage", config)) {
		server.tool(
			...
			"[WRITE] Add a persistent storage mount to a Coolify database",
			{
				uuid: schemas.uuid.describe("UUID of the database"),
	...
	if (isToolAllowed("coolify_delete_database_storage", config)) {
		server.tool(
			...
			"[DESTRUCTIVE] Remove a persistent storage mount from a Coolify database",
			{
				uuid: schemas.uuid.describe("UUID of the database"),
