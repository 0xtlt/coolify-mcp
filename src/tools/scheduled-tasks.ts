...
export function registerScheduledTaskTools(
	...
	server.tool(
		"coolify_list_application_scheduled_tasks",
		"List all scheduled tasks for a Coolify application",
		{ uuid: schemas.uuid.describe("UUID of the application") },
		async ({ uuid }) => {
			return wrap(async () => {
				const tasks = await client.listApplicationScheduledTasks(uuid);
				return tasks.map(toScheduledTaskSummary);
			});
		},
	);

	if (isToolAllowed("coolify_create_application_scheduled_task", config)) {
		server.tool(
			...
			async ({ uuid, name, command, frequency, container, timeout, enabled }) => {
				if (!isToolAllowed("coolify_create_application_scheduled_task", config))
					return readonlyError("coolify_create_application_scheduled_task");
				return wrap(async () => {
					const result = await client.createApplicationScheduledTask(uuid, {
						name,
						command,
						frequency,
						container,
	...
	if (isToolAllowed("coolify_update_application_scheduled_task", config)) {
		server.tool(
			...
			async ({ uuid, task_uuid, ...fields }) => {
				if (!isToolAllowed("coolify_update_application_scheduled_task", config))
					return readonlyError("coolify_update_application_scheduled_task");
				return wrap(async () => {
					const data: Record<string, unknown> = {};
					for (const [k, v] of Object.entries(fields)) {
						if (v !== undefined) data[k] = v;
					}
					await client.updateApplicationScheduledTask(uuid, task_uuid, data);
	...
	if (isToolAllowed("coolify_delete_application_scheduled_task", config)) {
		server.tool(
			...
			async ({ uuid, task_uuid, confirm }) => {
				if (!isToolAllowed("coolify_delete_application_scheduled_task", config))
					return readonlyError("coolify_delete_application_scheduled_task");
				...
				return wrap(async () => {
					const result = await client.deleteApplicationScheduledTask(uuid, task_uuid);
